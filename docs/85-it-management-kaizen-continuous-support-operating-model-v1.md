...
- 「IT部長代替」を継続KAIZEN支援の顧客向けポジショニングにしない。
- IT部長・IT責任者の有無だけでICPを限定しない。

---

## 12. Value and Pricing Validation Principles — DECIDED

継続KAIZEN支援の価格は、IT顧問の時間単価や会議回数だけから決定しない。顧客が継続的に持つ「IT経営KAIZEN機能」と、実際に生まれた経営価値の両面から検証する。

価値検証は主に次の5方向で行う。

1. **IT経営機能を持つ**
   - 会社のITについて、経営とITをつなぎ、継続的に判断・改善する機能を持てているか。
2. **IT投資を最適化する**
   - 不要・重複・過剰なIT支出を見直し、必要な投資へ再配分できているか。
3. **社員の時間を生み出す**
   - ITや業務に起因する不要なWorkを減らし、社員が本来の仕事へ使える時間を増やせているか。
4. **会社を守る**
   - 重要なIT・Security・事業継続上のリスクを経営判断へつなぎ、必要なACTを実行できているか。
5. **ITを成長へ使う**
   - FUTUREから逆算し、IT・AI・自動化・データ等を会社の成長へ活用できているか。

これらはすべてを金額換算することを目的としない。削減額・回避額・削減Work等、確認できるものはFACTとして蓄積し、リスク低減や成長能力等は根拠なく金額効果を断定しない。

### Internal Business Case — DECIDED

ITを戦略的に考えられる部長職・IT責任者等を採用・配置した場合の人件費・採用費等との比較は、継続KAIZEN支援の価格妥当性を検証するための**内部Business Caseの一つ**として使用できる。

ただし、これは顧客向けの商品定義・ICP・営業ポジショニングではない。

> **「IT部長を外部化する」「IT部長の代わりになる」サービスとは定義しない。**

IT部長・IT責任者が存在する企業でも、会社の未来から逆算してITを考え、実際の変化を継続的に確認し、次の改善を経営判断する仕組みが十分に確立されていなければ、継続KAIZEN支援の対象になり得る。

したがって、ICPの本質は「IT責任者がいない企業」ではなく、以下とする。

> **ITを経営に活かして会社を良くしたいが、それを継続的に実行する仕組みが十分に確立されていない企業。**

従業員規模、IT責任者の有無、IT支出額、企業の成長・変化量等は、適合性やBusiness Caseを確認するための情報であり、それ単独を絶対条件としない。

---

## 13. Pricing Hypothesis — NOT DECIDED

現時点では、標準月額価格はBusiness Decisionしていない。

検証中の価格候補として、月額20万円・25万円等があるが、Canonical Priceではない。

特に月額25万円（年額300万円）は、以下の観点から継続検証する。

- 顧客が受け取るIT経営KAIZEN機能との整合
- 5方向の顧客価値
- Assessmentとの価格関係
- Delivery Economics
- FACTACTによるHuman Work削減・再利用・継続性
- 初期顧客で実際に発生するHuman Work
- 顧客が実際にDecisionする価格・理由

価格をHuman Hoursから直接算出しない。一方でHuman WorkはDelivery Capacity、粗利、Scale可能性を検証するためのFACTとして計測する。

---

## 14. Next Business Design

次のBusiness Decision対象は、継続KAIZEN支援について以下を商品化・検証することである。
...
8. 初期ICPと顧客Business Case
9. Human Work / Delivery Capacityの実測方法
